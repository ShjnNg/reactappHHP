...
export default function PostList({ text }) {
  ...
  const getProductsByType = useSelector((state) => state.getProductsByType);
  const { loading, error,products  } = getProductsByType; 
  
  useEffect(() => {
    dispatch(listProducts(decodeURIComponent(text.match.params.type)));
  }, [dispatch ]);
